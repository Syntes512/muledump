...
setuptools.lightbox.menu.paginate.create = function(PageList, ActionItem, ActionContainer, ActionSelector, ActionCallback, ActionContext, Modifiers) {
    ...
    state.html.menu = ' \
        <div class="setuptools div pageControls full"> \
            ...
            <div class="editor control customPage"><input name="customPage" value="' + ( (state.currentPage > -1) ? state.currentPage+1 : 1 ) + '"><span class="noselect"> из ' + (state.lastPage+1) + ' страниц' + ( ((state.lastPage+1) < 2) ? 'ы' : '' ) + '</span></div> \
            <div class="editor control gotoPage noselect" title="Перейти">&#x1F50E</div> \
            \
            <div class="editor control lastPage noselect" title="Последняя страница">&#187;</div> \
...
setuptools.lightbox.display = function(page, config) {
    ...
    //  create the lightbox from the build data
        ...
        if ( ['drawhelp', 'main', 'accounts-deepcopy-download', 'settings-eraseAll-complete', 'settings-erase-completed'].indexOf(page) === -1 ) {
            ...
            setuptools.lightbox.builds[page].push('<a href="#" class="drawhelp gohome" title="Домашняя страница"' + (( typeof drawhelpData === 'object' ) ? ' style="right: ' + widthOffset + 'px;"' : '') + '>&#9751;</a>');
        }
