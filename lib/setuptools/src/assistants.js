    ...
    if ( manual === true || !lastBackup || (Date.now()-lastBackup >= (setuptools.data.config.backupAssistant*86400000)) ) {
        ...
        setuptools.lightbox.build('backup-assistant', ' \
            Хорошей практикой считается периодическое скачивание резервной копии Muledump.\
             \
            <br><br><div class="setuptools link downloadBackup menuStyle menuSmall cfleft" style="text-align: center;">Скачать резервную копию</div> \
...
setuptools.app.assistants.jakcodexcors = function() {
    ...
    setuptools.lightbox.build('jakcodexcors-extension-notice', " \
        ...
        <br><br>Более подробно вы сможете узнать на <a href='https://github.com/jakcodex/muledump/wiki/Muledump+CORS+Adapter' target='_blank'>Muledump CORS Adapter</a>, на wiki-страничке.\
    ");
...
setuptools.app.assistants.browser = function() {
    ...
    setuptools.lightbox.build('browser-assistant', ' \
            ...
            <div><br>Мы рекомендуем вам переключиться на браузер Google Chrome или Mozilla Firefox, когда вы будете готовы, вы можете просмотреть <a href="https://github.com/jakcodex/muledump/wiki/Installation-and-Setup" target="_blank">инструкцию по установке</a> для более полного понимания.</div> \
        ');
