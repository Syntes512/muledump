...
setuptools.app.config.settings = function(highlight, section) {
    ...
    if ( setuptools.data.config.enabled === true ) {
        ...
        setuptools.lightbox.build('settings', ' \
                <div class="setuptools app config settings"> \
                    <div id="settings-accountLoadDelay">Время задержки перед загрузкой аккаунта</div> \
                    <select name="accountLoadDelay" class="setting"> \
        ');

        for (i = 0; i <= 60; i++) setuptools.lightbox.build('settings', ' \
                <option ' + ( (Number(i) === Number(setuptools.data.config.accountLoadDelay)) ? 'selected' : '' ) + ' value="' + i + '">' + (( i === 0 ) ? 'Автоматически' : i) + '</option> \
        ');
        ...
        setuptools.lightbox.build('settings', ' \
                ...
                <div class="setuptools app config settings"> \
                    <div id="settings-mqAnimations">Анимация MuleQueue</div> \
                    <select name="mqAnimations" class="setting"> \
                        <option' + ( (setuptools.data.config.mqAnimations === false) ? ' selected' : '' ) + ' value="0">Уменьшенно</option> \
                        <option' + ( (setuptools.data.config.mqAnimations === true) ? ' selected' : '' ) + ' value="1">Полностью</option> \
                    </select> \
                </div> \
                <div class="setuptools app config settings"> \
