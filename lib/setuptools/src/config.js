...
setuptools.app.config.settings = function(highlight, section) {
    ...
    setuptools.lightbox.build('settings', ' \
        ...
        <div class="setuptools containerSettings muledump' + ( (section !== 'muledump') ? ' hidden' : '' ) + '">\
            ...
            <div class="setuptools app config settings"> \
                <div id="settings-accountLoadDelay">Задержка загрузки аккаунтов</div> \
    ...
    setuptools.lightbox.build('settings', ' \
            ...
            <div class="setuptools app config settings"> \
                <div id="settings-rowlength">Персонажи отображаемые в линию</div> \
    ...
    setuptools.lightbox.build('settings', ' \
            ...
            <div class="setuptools app config settings"' + disabled + '> \
                <div id="settings-mulelogin">Включить логин в один клик</div> \
                <select name="mulelogin" class="setting">\
                    <option' + ( (setuptools.data.config.mulelogin === 1) ? ' selected' : '' ) + ' value="1">Да</option> \
                    <option' + ( (setuptools.data.config.mulelogin === 0) ? ' selected' : '' ) + ' value="0">Нет</option> \
                </select> \
            </div> \
            <div class="setuptools app config settings"> \
                <div id="settings-nomasonry">Использовать умную сетку</div> \
                <select name="nomasonry" class="setting">\
                    <option' + ( (setuptools.data.config.nomasonry === 0) ? ' selected' : '' ) + ' value="0">Да</option> \
                    <option' + ( (setuptools.data.config.nomasonry === 1) ? ' selected' : '' ) + ' value="1">Нет</option> \
                </select> \
            </div> \
            <div class="setuptools app config settings"> \
                <div id="settings-testing">Тестирование</div> \
                <select name="testing" class="setting">\
                    <option' + ( (setuptools.data.config.testing === 1) ? ' selected' : '' ) + ' value="1">Да</option> \
                    <option' + ( (setuptools.data.config.testing === 0) ? ' selected' : '' ) + ' value="0">Нет</option> \
                </select> \
            </div> \
            <div class="setuptools app config settings"> \
                <div id="settings-totalswidth">Ширина общей сводки</div> \
                <select name="totalswidth" class="setting"> \
                <option value="0">Весь экран</option> \
    ');
    ...
    setuptools.lightbox.build('settings', ' \
            ...
            <div class="setuptools link config settings action save menuStyle cfleft noclose">Сохранить</div>\
        </div>\
        \
        <div class="setuptools containerSettings settings' + ( (section !== 'settings') ? ' hidden' : '' ) + '"> \
            <h3>SetupTools Settings</h3><br> \
            <div class="setuptools app config settings"> \
                <div id="settings-enabled">Помощник активен</div> \
                <select name="enabled" class="setting"' + ( (setuptools.state.hosted === true) ? ' disabled' : '' ) + '>\
                    <option' + ( (setuptools.data.config.enabled === true) ? ' selected' : '' ) + ' value="1">Да</option> \
                    <option' + ( (setuptools.data.config.enabled === false) ? ' selected' : '' ) + ' value="0">Нет</option> \
                </select> \
            </div> \
    ');
    ...
    if ( setuptools.data.config.enabled === true ) {
        ...
        setuptools.lightbox.build('settings', ' \
                <div class="setuptools app config settings"> \
                    <div id="settings-accountsPerPage">Аккаунты отображаемые за страницу</div> \
        ...
        if (setuptools.config.devForcePoint === 'online-versioncheck' || setuptools.state.hosted === true) {
            ...
            setuptools.lightbox.build('settings', ' \
                    <div class="setuptools app config settings"> \
                        <div id="settings-alertNewVersion">Уведомление о новой версии</div> \
                        <select name="alertNewVersion" class="setting">\
                            <option' + ( (setuptools.data.config.alertNewVersion === 0) ? ' selected' : '' ) + ' value="0">Off</option> \
                            <option' + ( (setuptools.data.config.alertNewVersion === 1) ? ' selected' : '' ) + ' value="1">On, только готовые</option> \
                            <option' + ( (setuptools.data.config.alertNewVersion === 2) ? ' selected' : '' ) + ' value="2">On, включая тестовые</option> \
                        </select> \
                    </div> \
            ');
        ...
        setuptools.lightbox.build('settings', ' \
                <div class="setuptools app config settings"> \
                    <div id="settings-animations">Анимация</div> \
                    <select name="animations" class="setting"> \
                        <option' + ( (setuptools.data.config.animations === false) ? ' selected' : '' ) + ' value="0">Урезанная</option> \
                        <option' + ( (setuptools.data.config.animations === true) ? ' selected' : '' ) + ' value="1">Полная</option> \
                    </select> \
                </div> \
                <div class="setuptools app config settings"> \
                    <div id="settings-automaticBackups">Автоматическое резервное копирование</div> \
                    <select name="automaticBackups" class="setting">\
                        <option' + ( (setuptools.data.config.automaticBackups === true) ? ' selected' : '' ) + ' value="1">Да</option> \
                        <option' + ( (setuptools.data.config.automaticBackups === false) ? ' selected' : '' ) + ' value="0">Нет</option> \
                    </select> \
                </div> \
                <div class="setuptools app config settings"> \
                    <div id="settings-autoReloadDays">Автоматически перезагружать</div> \
                    <select name="autoReloadDays" class="setting"> \
                        <option value="0">Off</option> \
        ');

        for (i = 1; i <= 14; i++) setuptools.lightbox.build('settings', ' \
                        <option ' + ( (Number(i) === Number(setuptools.data.config.autoReloadDays)) ? 'selected' : '' ) + ' value="' + i + '">' + i + ' ' + ( (i === 1) ? 'день' : 'дней' ) + '</option> \
        ');
        ...
        setuptools.lightbox.build('settings', ' \
                ...
                <div class="setuptools app config settings"> \
                    <div id="settings-debugging">Журнал отладки</div> \
                ...
                <div class="setuptools app config settings"> \
                    <div id="settings-groupsMergeMode">Режим менеджера групп</div> \
                    <select name="groupsMergeMode" class="setting"> \
                        <option' + ( (setuptools.data.config.groupsMergeMode === 0) ? ' selected' : '' ) + ' value="0">Off</option> \
                        <option' + ( (setuptools.data.config.groupsMergeMode === 1) ? ' selected' : '' ) + ' value="1">On, параллельный</option> \
                        <option' + ( (setuptools.data.config.groupsMergeMode === 2) ? ' selected' : '' ) + ' value="2">On, последовательный</option> \
                    </select> \
                </div> \
                <!-- soon, 9.2 -- <div class="setuptools app config settings"> \
                    <div id="settings-loginOnlyTotals">Только логин</div> \
                    <select name="loginOnlyTotals" class="setting">\
                        <option' + ( (setuptools.data.config.loginOnlyTotals === true) ? ' selected' : '' ) + ' value="1">Да</option> \
                        <option' + ( (setuptools.data.config.loginOnlyTotals === false) ? ' selected' : '' ) + ' value="0">Нет</option> \
                    </select> \
                </div>--> \
                <div class="setuptools app config settings"> \
                    <div id="settings-longpress">Длительность нажатия</div> \
                    <select name="longpress" class="setting">\
                        <option' + ( (setuptools.data.config.longpress === 500) ? ' selected' : '' ) + ' value="500">0.5 секунды</option> \
                        <option' + ( (setuptools.data.config.longpress === 1000) ? ' selected' : '' ) + ' value="1000">1 секунда</option> \
                        <option' + ( (setuptools.data.config.longpress === 2000) ? ' selected' : '' ) + ' value="2000">2 секунды</option> \
                        <option' + ( (setuptools.data.config.longpress === 3000) ? ' selected' : '' ) + ' value="3000">3 секунды</option> \
                    </select> \
                </div> \
                <div class="setuptools app config settings"> \
                    <div>Максимум резервных копий</div> \
        ...
        setuptools.lightbox.build('settings', ' \
                ...
                <div class="setuptools app config settings"> \
                    <div>Позиция меню</div> \
                    <select name="menuPosition" class="setting">\
                        <option' + ( (setuptools.data.config.menuPosition === 0) ? ' selected' : '' ) + ' value="0">Слева</option> \
                        <option' + ( (setuptools.data.config.menuPosition === 1) ? ' selected' : '' ) + ' value="1">По центру</option> \
                        <option' + ( (setuptools.data.config.menuPosition === 2) ? ' selected' : '' ) + ' value="2">Справа</option> \
                    </select> \
                </div> \
                <div class="setuptools app config settings"> \
                    ...
                    <select name="pagesearch" class="setting">\
                        ...
                        <option' + ( (setuptools.data.config.pagesearch === 1) ? ' selected' : '' ) + ' value="1">On (Мини)</option> \
                        <option' + ( (setuptools.data.config.pagesearch === 2) ? ' selected' : '' ) + ' value="2">On (Полный)</option> \
                    </select> \
                </div> \
                <div class="setuptools app config settings"> \
                    <div>Предзагрузка</div> \
                    <select name="preventAutoDownload" class="setting"> \
                        <option' + ( (setuptools.data.config.preventAutoDownload === true) ? ' selected' : '' ) + ' value="1">Да</option> \
                        <option' + ( (setuptools.data.config.preventAutoDownload === false) ? ' selected' : '' ) + ' value="0">Нет</option> \
                    </select> \
                </div> \
        ');
    ...
    setuptools.lightbox.build('settings', ' \
        ...
        <div class="setuptools containerSettings assistants' + ( (section !== 'assistants') ? ' hidden' : '' ) + '"> \
            ...
            <div class="setuptools app config settings"> \
                <div id="settings-accountAssistant">Помощник по настройке аккаунтов</div> \
            ...
            <div class="setuptools app config settings"> \
                <div id="settings-backupAssistant">Помощник по резервному копированию</div> \
                <select name="backupAssistant" class="setting"> \
                    <option' + ( (setuptools.data.config.backupAssistant === 0) ? ' selected' : '' ) + ' value="0">Off</option>\
                    <option' + ( (setuptools.data.config.backupAssistant === 7) ? ' selected' : '' ) + ' value="7">1 неделя</option> \
                    <option' + ( (setuptools.data.config.backupAssistant === 14) ? ' selected' : '' ) + ' value="14">2 недели</option> \
                    <option' + ( (setuptools.data.config.backupAssistant === 21) ? ' selected' : '' ) + ' value="21">3 недели</option> \
                    <option' + ( (setuptools.data.config.backupAssistant === 28) ? ' selected' : '' ) + ' value="28">4 недели</option> \
                </select> \
            </div> \
            <div class="setuptools app config settings"> \
                <div id="settings-corsAssistant">CORS помощник</div> \
    ...
    setuptools.lightbox.build('settings', ' \
            <div class="fleft cboth">\
                ...
                <div class="setuptools link config settings action resetOptions menuStyle negative cfleft noclose mt5">Сбросить опции</div> \
                <div class="setuptools link config settings action arrrrgbaaad menuStyle negative cfleft noclose mt5">Сбросить помощника</div> \
                <div class="setuptools link config settings action eraseCache menuStyle negative cfleft noclose mt5">Удалить кэш</div> \
                <div class="setuptools link config settings action eraseAll menuStyle negative cfleft noclose mt5">Удалить все данные</div> \
            </div>\
        </div> \
    ');

    setuptools.lightbox.build('settings', ' \
            <div class="setuptools link config settings action destroy menuStyle negative cfleft noclose mt5">Сброс к стандартным настройкам</div>\
        </div> \
    ');
    ...
    $('.setuptools.link.resetOptions').click(function() {
        ...
        setuptools.lightbox.build('settings-resetOptions-confirm', ' \
            Это приведет к сбросу всех глобальных параметров и параметров учетной записи в Muledump. \
            <br><br>Хотите продолжить? \
            <br><br><div class="setuptools link resetOptions confirm menuStyle menuSmall negative cfleft">Да, стереть</div>\
            <div class="setuptools link cancelBadThing menuStyle menuSmall cfright">Нет, отмена</div> \
            <br><br>&nbsp;\
        ');
        ...
        $('.setuptools.link.resetOptions.confirm').click(function() {
            ...
            try {
                ...
                setuptools.lightbox.build('settings-resetOptions-error', 'Возникла ошибка во время сброса.');
            ...
            options_init();
            window.relayout();
            setuptools.lightbox.build('settings-resetOptions-complete', 'Все настройки сброшены.');
    ...
    $('.setuptools.link.config.settings.action.eraseCache').click(function() {
        ...
        setuptools.lightbox.build('settings-eraseCache-confirm', ' \
            Это приведет к удалению всех кэшированных данных учетной записи, хранящихся в вашем браузере. \
            <br><br>Хотите продолжить? \
            <br><br><div class="setuptools link eraseCache confirm menuStyle menuSmall negative cfleft">Да, стереть</div>\
            <div class="setuptools link cancelBadThing menuStyle menuSmall cfright">Нет, отмена</div> \
        ');
        ...
        $('.setuptools.link.eraseCache.confirm').click(function() {
            ...
            setuptools.lightbox.build('settings-eraseCache-complete', 'Все кэшированные данные были удалены. Перезагрузите Muledump.');
    ...
    $('.setuptools.link.config.settings.action.eraseAll').click(function() {
        ...
        setuptools.lightbox.build('settings-eraseAll-confirm', ' \
            Это приведет к удалению всех данных Muledump. \
            <br><br>Хотите продолжить? \
            <br><br><div class="setuptools link eraseAll confirm menuStyle negative cfleft menuSmall">Да, удалить</div> \
            <div class="setuptools link cancelBadThing menuStyle cfright menuSmall">Нет, отмена</div> \
        ');
        ...
        $('.setuptools.link.eraseAll.confirm').click(function() {
            ...
            setuptools.lightbox.build('settings-eraseAll-complete', 'Все данные были удалены. Muledump будет автоматически перезагружен.');
    ...
    $('.setuptools.link.config.settings.action.arrrrgbaaad').click(function() {
        ...
        setuptools.lightbox.build('settings-erase-confirm', ' \
            Это приведет к стиранию SetupTools и его конфигурации. Это не удалит accounts.js \
            <br><br>Хотите продолжить? \
            <br><br><div class="setuptools link erase menuStyle negative cfleft menuSmall">Да, стереть</div> \
            <div class="setuptools link cancelBadThing menuStyle cfright menuSmall">Нет, отмена</div> \
            <div class="fleft cboth"><br><input type="checkbox" name="clearBackups"> Удалить все резервные копии</div> \
        ');
        ...
        $('.setuptools.link.erase').click(function () {
            ...
            setuptools.lightbox.build('settings-erase-completed', ' \
                SetupTools конфигурация была удалена. \
                ' + ( (BackupList.length > 0 && BackupKeys.length === 0 ) ? '<br><br>Удалено ' + BackupList.length + ' резервных копий.' : '') + ' \
                <br><br>Это окно будет перезагружено через несколько секунд. \
            ');
    ...
    $('.setuptools.link.config.settings.save').click(function() {
        ...
        if ( setuptools.app.config.save('SettingsManager/Save') === false ) {
            ...
            setuptools.lightbox.build('settings-saved', 'Настройки успешно сохранены. <br><br>Это окно будет перезагружено через несколько секунд.');
...
setuptools.app.config.downloadAck = function() {
    ...
    $('.setuptools.config.download').click(function(e) {
        ...
        if ( setuptools.data.config.preventAutoDownload === true ) {
            ...
            if (AckCount == Number(50)) $('.setuptools.config.download.acknowledge').html('Ты действительно развлекаешься этим, не так ли?');
            if (AckCount == Number(100)) $('.setuptools.config.download.acknowledge').html('Да, я так и думал. <a href="#" data-featherlight="http://3.bp.blogspot.com/_D_Z-D2tzi14/S8TTPQCPA6I/AAAAAAAACwA/ZHZH-Bi8OmI/s400/ALOT2.png">Мне нравится это еще больше</a>.');
            AckCount++;
