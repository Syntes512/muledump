...
setuptools.app.mulequeue.task.rateLimit = function(soft) {
    ...
    function RateLimitTimer() {
        ...
        if (RateLimitRemaining <= 0) {
            ...
            $('#stickynotice').html("<div>Внимание: Вы достигли ограничения, пожалуйста подождите " + RateLimitTime + " прежде чем перезагружать аккаунты.</div>");
...
setuptools.app.mulequeue.task.start = function(guid, callback) {
    ...
    if ( setuptools.data.config.animations === true ) setuptools.app.mulequeue.tasks[guid].reloader.empty().append(setuptools.app.mulequeue.spinner.running('running')).attr('title', 'Запрос данных');
    $('div.queuetask[data-listPos="' + taskIndex + '"] > div:first-child').html(setuptools.app.mulequeue.spinner.running('running', true)).attr('title', 'Запрос данных');
...
setuptools.app.mulequeue.menu = function() {
    ...
    var options = [
        ...
        {
            class: 'openManager',
            name: setuptools.app.mulequeue.queue.length + ' Задач ' + ( (setuptools.app.mulequeue.state.paused === false) ? 'запущено' : 'приостановлено' )
        }
    ];
...
setuptools.app.mulequeue.ui.drawButtons = function() {
    ...
    if ( setuptools.app.mulequeue.state.running === false ) {
        ...
        html += ' \
            <div class="setuptools menuStyle buttonStyle positive mulequeue main menu reload noselect" title="Перезагрузить все аккаунты" style="float: right; padding: 0; display: flex; justify-content: center; align-items: center;"><div style="font-size: 14px; font-weight: normal;">&#9654;</div></div> \
                    ';
        ...
        if ( setuptools.app.mulequeue.state.paused === false ) {
            ...
            html += ' \
                <div class="setuptools menuStyle buttonStyle mulequeue main menu pause noselect" title="Приостановить" style="float: right; font-size: 10px; font-weight: normal;">&#10073;&#10073;</div> \
            ';
            ...
            //  reload &#8634;
            html += ' \
                <div class="setuptools menuStyle buttonStyle mulequeue main menu resume noselect" title="Продолжить" style="float: right; font-weight: normal;">&#9654;</div> \
            ';

        }

        html += ' \
            <div class="setuptools menuStyle buttonStyle negative mulequeue main menu cancel noselect" title="Отмена" style="float: right; font-size: 10px; font-weight: normal;">&#10006;</div> \
        ';
    ...
    function createRow(listPos) {
        ...
        return ' \
            <div class="flex-container queuetask noselect" style="justify-content: flex-start;" data-listPos="' + listPos + '" data-guid="' + QueueList[listPos] + '"> \
                <div style="flex-basis: 27px; height: 24px;" class="flex-container textCenter" title="Позиция в очереди: ' + (listPos+1) + '">' + setuptools.app.mulequeue.spinner.running(undefined, true) + '</div> \
                ...
                <div style="flex-basis: 30px; height: 24px; justify-content: flex-end;" class="flex-container">\
                    <div style="flex-basis: 25px;" class="flex-container setuptools menuStyle buttonStyle link taskMenu noclose mr0" title="Изменение приоритета">&#8801;</div> \
                </div>\
            </div>\
        ';
...
setuptools.app.mulequeue.ui.progressBar = function(jobsRemaining, jobsTotal) {
    ...
    if ( jobsRemaining !== 0 ) {
        ...
        $('.mulequeue.progress > .percentage').text(jobsRemaining + ' осталось (' + percentage + '% выполнено)');

    } else {

        $('.ldBar').attr('data-value', 0);
        $('.mulequeue.progress > .bar').css({width: 0 + '%'});
        $('.mulequeue.progress > .percentage').text('Нет задач');
