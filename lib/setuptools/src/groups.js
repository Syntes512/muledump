...
setuptools.app.groups.manager = function(group, open) {
    ...
    setuptools.lightbox.build('groups-manager', ' \
        <div class="Groups-ManagerMenu fleft cboth">\
            <div class="setuptools link groups add menuStyle cfleft">Создать группу</div> \
            <div class="setuptools link groups enableAll toggle noclose menuStyle fleft">Включить все</div> \
            <div class="setuptools link groups disableAll toggle noclose menuStyle fleft">Выключить все</div> \
        </div>\
        <div class="setuptools div groups container cfleft"> \
        <br><strong>Существующие группы:</strong> \
        <br> \
    ');
    ...
    if ( typeof setuptools.data.groups.format === 'undefined' ||  Object.keys(setuptools.data.groups.groupList).length === 0 ) {
        ...
        setuptools.lightbox.build('groups-manager', 'Группы не найдены.');

    } else {

        setuptools.lightbox.build('groups-manager', ' \
            <div class="setuptools div groupControls"> \
                <div class="groupMenu" title="Меню группы">&#8801;</div> \
                <div class="groupSelect" title="Меню выделения">&#10967;</div> \
            </div> \
    ...
    setuptools.lightbox.build('groups-manager', ' \
        <div class="setuptools div groupEditor container noselect"> \
            <br><h3>Редактор групп</h3> \
            <div class="setuptools div groups editor">Выберите или создайте группу.</div> \
            </div> \
        </div>\
    ');
    ...
    $('.setuptools.link.groups.toggle').click(function() {
        ...
        if ( setuptools.app.config.save() === true ) {
            ...
            $(this).html('<span class="success">Сохранено!</span>');
    ...
    function OpenEditor(groupName, self) {
        ...
        var ListAccountTiles = function(groupName, page) {
            ...
            if ( html.length === 0 ) {
                html += '<div class="cell noselect nocontext">Нет доступных аккаунтов</div>';
            }
        ...
        var ListGroupMembers = function(groupName, page) {
            ...
            if ( html.length === 0 ) {
                html += '<div class="cell noselect nocontext">Аккаунты не добавлены</div>';
            }
        ...
        $('.setuptools.div.groupEditor.container > h3').text('Редактор групп - ' + groupName + ' (' + ( (setuptools.data.groups.groupList[groupName].enabled === true) ? 'Активна' : 'Выключена' ) + ')');
        ...
        editor.html(' \
            ...
            <div class="setuptools groupSaveControls fleft w100 cboth" style="margin-top: 34px;"> \
                ...
                <div class="setuptools revert group menuStyle negative fright mr0">Отменить изменения</div> \
                \
                <div class="controller notifier empty saveNotice menuStyle cfleft">&nbsp;</div> \
                <div class="setuptools close group menuStyle negative fright mt5 mr0">Закрыть редактор</div> \
            </div>\
        ');
        ...
        $('.setuptools.save.group').click(function() {
            ...
            if ( setuptools.app.config.save() === true ) {
                ...
                $('div.save.result').html('<div>Сохранено!</div>');
    ...
    function CloseEditor() {
        ...
        $('.setuptools.div.groups.editor').text('Выберите или создайте группу.');
        ...
        //  single group menu
            ...
            if ( $('.setuptools.div.groupControls div.groupMenu').hasClass('selected') === false ) {
                ...
                if ( $('.setuptools.div.groupList div.dragbox div.cell:not(.selected)').length > 0 ) {
                    ...
                    options.push({
                        class: 'moveRight',
                        name: 'Переместить вправо',
                        callback: MoveRight
                    });

                    options.push({
                        class: 'moveToEnd',
                        name: 'Переместить в конец',
                        callback: MoveToEnd
                    });
    ...
    $('.setuptools.div.groupControls div.groupSelect').click(function (e) {
        ...
        if ( $('.setuptools.div.groupControls div.groupSelect').hasClass('selected') === false ) {
            ...
            var options = [
                ...
                {
                    class: 'selectNone',
                    name: 'Убрать выделение',
                    callback: SelectNone
                },
                {
                    class: 'selectEnabled',
                    name: 'Выбрать активные группы',
                    callback: SelectAllEnabled
                },
                {
                    class: 'selectDisabled',
                    name: 'Выбрать неактивные группы',
...
setuptools.app.groups.delete = function(groupName) {
    ...
    if ( groupNames.length < 2 ) setuptools.lightbox.build('groups-delete', 'Вы уверены в том, что хотите удалить эту группу?');
    if ( groupNames.length > 1 ) setuptools.lightbox.build('groups-delete', 'Вы уверены в том, что хотите удалить эти группы?');
    ...
    setuptools.lightbox.build('groups-delete', ' \
        <br><br>' + groupList + ' \
        <br><br><div class="setuptools link confirm menuStyle menuSmall cfleft">Да, удалить</div> <div class="setuptools link cancel menuStyle menuSmall negative cfright">Нет, отмена</div>\
    ');
...
setuptools.app.groups.copy = function(groupName, newGroupName) {
    ...
    if ( typeof setuptools.data.groups.groupList[groupName] === 'undefined' ) {
        ...
        setuptools.lightbox.build('groups-copy', 'Указанная группа "' + groupName + '" не найдена.');

    } else {

        if (newGroupName) setuptools.lightbox.build('groups-copy', ' \
            Новое название уже используется.<br><br> \
        ');

        if (!newGroupName) newGroupName = '';

        setuptools.lightbox.build('groups-copy', ' \
            Укажите название для новой группы <br><br> \
            <div class="w400px">\
                <div class="setuptools div app groups">\
                    <div><strong>Название группы</strong></div> \
                    <input name="groupName" value="' + newGroupName + '"> \
                </div> \
                <div class="setuptools link save cfright formStyle mt15">Создать группу</div> \
            </div>\
        ');
...
setuptools.app.groups.rename = function(groupName, newGroupName) {
    ...
    if ( typeof setuptools.data.groups.groupList[groupName] === 'undefined' ) {
        ...
        setuptools.lightbox.build('groups-rename', 'Указанная группа "' + groupName + '" не существует.');
        ...
        if ( typeof newGroupName === 'string' ) {
            ...
            if ( newGroupName.length < 1 ) {
                setuptools.lightbox.build('groups-rename', ' \
                    Название группы не может быть пустым.<br><br> \
                ');
            } else {
                setuptools.lightbox.build('groups-rename', ' \
                    Данное название группы уже занято.<br><br> \
                ');
            }
        }
        ...
        setuptools.lightbox.build('groups-rename', ' \
            Укажите новое название группы "' + groupName + '". <br><br> \
            <div class="w400px">\
                <div class="setuptools div app groups">\
                    <div><strong>Название группы</strong></div> \
                    <input name="groupName" value="' + newGroupName + '"> \
                </div> \
                <div class="setuptools link save cfright formStyle mt15">Переименовать</div> \
            </div>\
        ');
...
setuptools.app.groups.merge = function(selectedClass, newGroupName) {
    ...
    if ( selectedClass.length === 0 ) {
        ...
        setuptools.lightbox.build('groups-merge', ' \
            <div class="w400px">\
                <div class="setuptools div app groups">\
                    <div><strong>Имя объединяемой группы</strong></div> \
                    <input name="groupName" value="' + newGroupName + '"> \
                </div> \
                <div class="setuptools div app groups"> \
                    <div><strong>Сохранить исходные группы</strong></div> \
                    <select name="keepGroups"> \
                        <option value="0">Нет</option> \
                        <option value="1">Да</option> \
                    </select>\
                </div> \
                <div class="setuptools link save cfright formStyle mt15">Совместить группы</div> \
            </div>\
        ');
...
setuptools.app.groups.add = function(groupName) {
    ...
    setuptools.lightbox.build('groups-add', ' \
        <div class="w400px">\
            ...
            <div class="setuptools div app groups">\
                <div><strong>Приоритет группы</strong></div> \
        ...
        setuptools.lightbox.build('groups-add', ' \
            ...
            <div class="setuptools link save cfright formStyle mt15">Создать</div> \
        </div>\
    ');
