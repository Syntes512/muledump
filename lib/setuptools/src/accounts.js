...
setuptools.app.accounts.start = function() {
    ...
    setuptools.lightbox.build('accounts-start', '<span>У тебя есть <a href="#" class="setuptools initsetup page2">один или несколько аккаунтов</a> или <a href="#" class="setuptools initsetup page3">много аккаунтов</a>?</span>');
...
setuptools.app.accounts.manager = function() {
    ...
    function createRow(listPos) {
        ...
        return ' \
            <div class="cell noselect" data-listPos="' + listPos + '"> \
                <div><input class="text" name="username" type="text" placeholder="Почта или Steam-ID" value="' + AccountsList[listPos].username + '"></div> \
                <div><input class="text" name="password" type="password" placeholder="Пароль" value="' + AccountsList[listPos].password + '" data-reveal="false"></div> \
                <div><div class="setuptools div accountControls">\
                    <div data-option="accountMenu" title="Меню">&#8801;</div> \
                    <div data-option="toggleEnable" class="' + (( AccountsList[listPos].enabled === true ) ? ' selected" title="Активен' : '" title="Не активен') + '" style="padding-top: 1px;">&#9733;</div> \
                    <div data-option="delete" title="Удалить аккаунт" style="padding-top: 1px;">&#10006;</div>\
                </div></div> \
            </div>\
        ';
    ...
    function AccountsContext() {
        ...
        $('div.editor.control.massSwitch').unbind('click').click(function() {
            ...
            setuptools.lightbox.build('accounts-massSwitch', ' \
                Включите или отключите настройки для всех учетных записей в этом меню. <br><br>\
                \
                <div class="massSwitch w100">\
                    <div class="cfleft"><h3>Аккаунты</h3></div> \
                    <div class="setuptools link menuStyle negative menuTiny cfright noclose" data-key="enabled" data-value="false">Отключить все</div> \
                    <div class="setuptools link menuStyle menuTiny fright noclose" data-key="enabled" data-value="true">Включить все</div> \
                    \
                    <div class="cfleft"><h3>Кэширование данных</h3></div> \
                    <div class="setuptools link menuStyle negative menuTiny cfright noclose" data-key="cacheEnabled" data-value="false">Отключить все</div> \
                    <div class="setuptools link menuStyle menuTiny fright noclose" data-key="cacheEnabled" data-value="true">Включить все</div>  \
                    \
                    <div class="cfleft"><h3>Авто-перезагрузка</h3></div> \
                    <div class="setuptools link menuStyle negative menuTiny cfright noclose" data-key="autoReload" data-value="false">Отключить все</div> \
                    <div class="setuptools link menuStyle menuTiny fright noclose" data-key="autoReload" data-value="true">Включить все</div>  \
                    \
                    <div class="cfleft"><h3>Только логин</h3></div> \
                    <div class="setuptools link menuStyle negative menuTiny cfright noclose" data-key="loginOnly" data-value="false">Отключить все</div> \
                    <div class="setuptools link menuStyle menuTiny fright noclose" data-key="loginOnly" data-value="true">Включить все</div> \
                \
                </div>\
            ');
            ...
            $('div.massSwitch > div.link').click(function() {
                ...
                $('.controller.notifier.saveNotice').removeClass('empty').addClass('notice').text('Не забудьте сохранить изменения').css({'cursor': 'initial'});
        ...
        $('div.setuptools.accountControls > div').unbind('click').click(function() {
            ...
            if ( name === 'accountMenu' ) {
                ...
                var options = [
                    {
                        option: 'header',
                        value: 'Опции аккаунта'
                    },
                    {
                        class: 'revealPassword',
                        name: ( (cell.find('input[name="password"]').attr('data-reveal') === 'true') ? 'Спрятать' : 'Показать' ) + ' пароль',
                        callback: function() {
                    ...
                    {
                        class: 'autoReload',
                        name: ( (AccountsList[listPos].autoReload === true) ? 'Выключить' : 'Включить' ) + ' автоматическую перезагрузку',
                    ...
                    {
                        class: 'cacheEnabled',
                        name: ( (AccountsList[listPos].cacheEnabled === true) ? 'Выключить' : 'Включить' ) + ' кэширование данных',
                        callback: function(listPos) {
                    ...
                    {
                        class: 'loginOnly',
                        name: ( (AccountsList[listPos].loginOnly === true) ? 'Выключить' : 'Включить' ) + ' только логин',
                        callback: function(listPos) {
    ...
    if ( setuptools.state.firsttime === false ) pageButtonsHtml = ' \
        <div class="editor control massSwitch noselect" title="Массовые переключатели" style="font-weight: normal; font-size: 12px; padding-top: 3px !important;">&#8801;</div> \
    ';
    ...
    setuptools.lightbox.build('accounts-manager', ' \
        <div class="setuptools accounts editor">\
            ...
            <div class="AccountsList"> \
                ...
                <div class="controller newAccount formStyle">Добавить аккаунт</div> \
                \
                <div class="controller saveAccounts menuStyle">Сохранить аккаунты <span id="changeStatus"></span></div> \
                <div class="controller importAccountsJS menuStyle">Импорт accounts.js</div> \
                \
                <div class="controller notifier empty saveNotice menuStyle cfleft">&nbsp;</div> \
                ' + ( ( setuptools.state.loaded === true ) ? '<div class="controller exportAccountsJS menuStyle cfright">Экспорт accounts.js</div>' : '' ) + ' \
            </div>\
        </div> \
    ');
    ...
    $('div.AccountsList > div.controller').click(function() {
        ...
        if ( $(this).hasClass('saveAccounts') === true ) {
            ...
            if ( Object.keys(setuptools.data.accounts.accounts).length === 0 ) {
                ...
                setuptools.lightbox.build('accounts-manager-emptySave', 'Не найдено доступных аккаунтов!');
                ...
                if ( setuptools.app.config.save() === false ) {
                    ...
                    setuptools.lightbox.build('accounts-manager-saveError', 'Произошла ошибка с вашей конфигурацией.');
                    ...
                    $('span#changeStatus').html('<span class="success">Успешно!</span>');
                    $('span#changeStatus > span').fadeOut(2500, function () {
                        $(this).remove();
                    });

                    $('.controller.notifier.saveNotice').removeClass('empty').addClass('notice').text('Перезагрузите для применения').css({cursor: 'pointer'}).click(function() {
                        setTimeout(function() {
...
setuptools.app.accounts.ExportDeepCopy = function(guid) {
        ...
        if ( typeof storageKey === 'undefined' ) {
            ...
            setuptools.lightbox.build('accounts-deepcopy-download', 'Не найден ' + guid + ', нет локальных данных. <br><br>Пожалуйста, загрузите аккаунт в Muledump и попробуйте снова');
            setuptools.lightbox.settitle('accounts-deepcopy-download', 'Полная копия');
        ...
        setuptools.lightbox.build('accounts-deepcopy-download', ' \
            Полная копия ' + guid + ' готова к загрузке. \
            <br><br><a download="' + FileName + '.json" href="data:text/json;base64,' + btoa(DeepCopyData) + '" class="setuptools config download noclose">Скачать</a> \
        ');
...
setuptools.app.accounts.saveConfirm = function() {
    ...
    setuptools.lightbox.build('accounts-saveConfirm', 'Успешно сохранено. \
        <br><br>Рекомендуется загрузить <a href="#" class="setuptools config backup">резервную копию</a> вашей конфигурации Muledump. \
        <br><br>Если вы готовы, вы можете приступить к работе с <a href="#" class="setuptools app loadaccounts">Muledump</a>. \
    ');
...
setuptools.app.accounts.AccountsJSExport = function() {
    ...
    setuptools.lightbox.build('accounts-accountsjs-export', ' \
        Accounts.js успешно создан. \
        <br><br><span class="setuptools config download acknowledge">Нажмите правой кнопкой мыши и выберите \'Save link as...\' для загрузки.</span> \
        <br><br><a download="accounts.js" href="data:text/javascript;base64,' + AccountsJSData + '" class="setuptools export accountsjs noclose">Скачать</a> \
    ');
...
setuptools.app.accounts.AccountsJSImport =  function() {
    ...
    setuptools.lightbox.build('accounts-accountsjs-index', ' \
        Есть два пути для импорта accounts.js. \
        <br><br>Вы можете поместить accounts.js в папку muledump; либо же загрузить его из другого места. \
        <br><br>Сначала мастер настройки должен быть включен. После включения вы сможете импортировать acounts.js. \
        <br><br><div class="setuptools link enable menuStyle menuSmall noclose fleft cboth">' + ( (setuptools.data.config.enabled === true) ? 'Мастер настройки активен' : 'Активировать мастер настройки' ) + '</div> \
    ...
    if ( Object.keys(setuptools.originalAccountsJS).length > 0 ) {
        ...
        setuptools.lightbox.build('accounts-accountsjs-index', ' \
            <div class="setuptools link import accountsJS ' + menuClass + ' menuSmall fleft mr5 noclose">Импорт accounts.js</div> \
            <div class="setuptools link import upload ' + menuClass + ' menuSmall fright noclose">Загрузка accounts.js</div> \
        ');

    } else {

        setuptools.lightbox.build('accounts-accountsjs-index', ' \
            <div class="setuptools link import upload ' + menuClass + ' menuSmall fleft noclose">Загрузка accounts.js</div> \
        ');
...
setuptools.app.accounts.AccountsJSImportLocal = function() {
    ...
    if ( FoundKeys > 0 ) {
        ...
        setuptools.lightbox.build('accounts-accountsjs-import', ' \
            Продолжить импорт с ' + FoundKeys + ' найденными аккаунтами в accounts.js? \
            <br><br><div class="setuptools link accounts importConfirmed menuStyle menuSmall cfleft">Обзор импорта</div> \
            <div class="setuptools link accounts importCancelled menuStyle menuSmall negative cfright">Отмена</div> \
            <div class="fleft cboth">\
                <br><input type="checkbox" name="accountjsMerge" checked> Убрать повторения. \
            </div>\
        ');
...
setuptools.app.accounts.AccountsJSImportUpload = function(manual) {
    ...
    function ParseUploadedFile(FileContents) {
        ...
        $('.setuptools.app.initsetup.uploadResults').html('Найдено ' + Object.keys(UploadParse.accounts).length + ' аккаунтов и ' + Object.keys(UploadParse.settings).length + ' настроек');
        ...
        $('.setuptools.link.uploadData.save').html('Подтвердить').click(function () {
            ...
            if ( Object.keys(UploadParse.accounts).length > 0 ) {
                ...
                $('.setuptools.app.initsetup.uploadResults').html('Сначала необходимо загрузить рабочий accounts.js.');
    ...
    if ( setuptools.config.devForcePoint !== 'restorejs-upload' && manual !== true && window.File && window.FileReader && window.FileList && window.Blob ) {
        ...
        setuptools.lightbox.build('accounts-accountsjs-upload', ' \
            Если у вас возникли какие-либо проблемы, вы можете переключиться в режим ручной загрузки. \
            <br><br>Пожалуйста выберите ваш accounts.js файл. \
        ...
        if ( typeof setuptools.tmp.FileReaderCapable === 'undefined' && manual !== true ) {
            ...
            setuptools.lightbox.build('accounts-accountsjs-upload', 'Загрузка файлов не поддерживается вашим браузером. Вместо этого вставьте содержимое файла accounts.js ниже.');

        } else {

            setuptools.lightbox.build('accounts-accountsjs-upload', 'Вставьте содержимое файла accounts.js ниже.');
    ...
    setuptools.lightbox.build('accounts-accountsjs-upload', ' \
        ...
        <div class="setuptools link uploadData save menuStyle cfright noclose nomargin">Выберите файл!</div> \
    ');
    ...
    if ( DoFiles === true ) {
        ...
        $('.setuptools.bottom.container').append(' \
            <div class="cfright mr5" style="height: 100%; margin-top: 3px;"> \
                <br><a href="#" class="setuptools app initsetup switchToUpload">Переключиться в режим ручной загрузки</a> \
            </div> \
        ');
