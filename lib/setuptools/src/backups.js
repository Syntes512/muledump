...
setuptools.app.backups.index = function() {
    ...
    if ( setuptools.state.loaded === true ) {
        ...
        //  for another time - <a href="#" class="setuptools app backups createdeep">Резервное копирование</a> | \
        setuptools.lightbox.build('backups-index', ' \
            <div class="fleft cboth w100" style="margin-bottom: 16px;">\
                <div class="setuptools link backups create menuStyle menuSmall textCenter cfleft">Создать</div> \
                <div class="setuptools link backups upload menuStyle menuSmall textCenter fleft">Загрузить</div> \
                <div class="setuptools link backups autobackups noclose fleft menuStyle menuSmall textCenter mr0 ' + ( (setuptools.data.config.automaticBackups === true) ? 'negative">OFF' : 'positive">ON' ) + ' Резервное копирование</div> \
            </div>\
        ');
        ...
        setuptools.lightbox.build('backups-index', ' \
            <div class="setuptools link backups upload menuStyle menuSmall cboth textCenter fleft" style="margin-bottom: 16px;">Загрузить</div> \
        ');
    ...
    if ( BackupList.length === 0 ) {
        ...
        setuptools.lightbox.build('backups-index', ' \
                <h3>Резервных копий не найдено.</h3> \
            </div>\
        ');
        ...
        setuptools.lightbox.build('backups-index', ' \
                <h3 class="fleft cboth" style="margin: 0 0 10px 0;">Сохраненные резервные копии</h3> \
        ...
        setuptools.lightbox.build('backups-index', ' \
                    ...
                    <div class="cfleft Backups-BackupOptions mt10"> \
                        <div class="setuptools link backups download menuStyle" title="Download Backup">Скачать</div> \
                        <div class="setuptools link backups rename menuStyle" title="Rename Backup">Название</div> \
                        <div class="setuptools link backups restore menuStyle" title="Restore Backup">Включить</div> \
                        <div class="setuptools link backups protect menuStyle" title="">&nbsp;</div> \
                        <div class="setuptools link backups delete menuStyle negative noclose" title="Delete Backup">Удалить</div> \
                    </div> \
                </div>\
            </div> \
        ');
    ...
    if ( setuptools.state.loaded === true ) {
        ...
        $('.setuptools.link.backups.autobackups').click(function() {
            ...
            if ( setuptools.app.config.save('BackupsManager/Toggle') === true ) {
                ...
                $(this).text((setuptools.data.config.automaticBackups === true) ? 'OFF Резервное копирование' : 'ON Резервное копирование');
    ...
    function updateProtection() {
        ...
        if ( typeof SelectedBackupData === 'object' && SelectedBackupData.meta ) {
            ...
            if ( SelectedBackupData.meta.protected === false ) {
                ...
                protectButton.text('Защита').attr('title', 'Enable Backup Protection');
                ...
                protectButton.text('Снять').attr('title', 'Disable Backup Protection');
...
setuptools.app.backups.upload = function(manual) {
    ...
    function ParseUploadedFile(FileContents) {
        ...
        if (typeof UploadFileJSON === 'object' && UploadFileJSON.meta) {
            ...
            $('.setuptools.link.backups.uploadData.save').html('Файл поврежден');
    ...
    if ( setuptools.config.devForcePoint !== 'backups-upload' && manual !== true && window.File && window.FileReader && window.FileList && window.Blob ) {
        ...
        setuptools.lightbox.build('backups-upload', ' \
            Выбери резервную копию, которую вы хотите восстановить. \
        ...
        if ( setuptools.state.loaded === true ) {
            ...
            setuptools.lightbox.build('backups-upload', ' \
                <br><br><input type="checkbox" name="restoreSaveExisting" checked title="Save Backup of Existing Configuration" style="width: auto;"> Сохранить резервную копию текущей конфигурации \
            ');
        ...
        if ( typeof setuptools.tmp.FileReaderCapable === 'undefined' && manual !== true ) {
            ...
            setuptools.lightbox.build('backups-upload', 'Загрузка файлов не поддерживается вашим браузером. Вместо этого вставьте содержимое файла резервной копии ниже.');

        } else {

            setuptools.lightbox.build('backups-upload', 'Пожалуйста, скопируйте содержимое резервной копии в поле ниже.');
        ...
        if ( setuptools.state.loaded === true ) {
            ...
            setuptools.lightbox.build('backups-upload', ' \
                <br><input type="checkbox" name="restoreSaveExisting" checked title="Save Backup of Existing Configuration" style="width: auto;"> Сохранить резервную копию текущей конфигурации \
            ');
    ...
    setuptools.lightbox.build('backups-upload', ' \
        ...
        <br><div class="setuptools link backups uploadData save menuStyle menuSmall cfright noclose mr0">Выбрать</div> \
    ');
    ...
    if ( DoFiles === true ) {
        ...
        $('.setuptools.bottom.container').append(' \
            <div class="cfright h100" style="margin-top: 3px;"> \
                <br><a href="#" class="setuptools app backups switchToUpload">Режим ручной загрузки</a> \
            </div> \
        ');
        ...
        $('.setuptools.link.backups.uploadData.save').html('Сохранить');
        $('textarea[name="uploadData"]').change(function() {

            $('.setuptools.link.backups.uploadData.save').html('Загрузка');
...
setuptools.app.backups.download = function(BackupID, BackupName, BackupFileName, BackupCustomName) {
    ...
    if ( BackupData ) {
        ...
        setuptools.lightbox.build('backups-download', ' \
            ...
            <br><br><a download="' + FileName + '.json" href="data:text/json;base64,' + btoa(BackupData) + '" class="setuptools config download noclose">Скачать</a> \
        ');
...
setuptools.app.backups.renameHtml = function(page, BackupID, BackupDataJSON, callback, noclose) {
    ...
    setuptools.lightbox.build(page, ' \
        <br><br><div class="setuptools app fleft cboth w100"> \
            <input name="BackupCustomName" value="' + ( (typeof BackupData.meta.name === 'string') ? BackupData.meta.name : '' ) + '" placeholder="Название резервной копии" class="cfleft mr5"> \
            <div class="setuptools link customBackupName formStyle noclose">Переименовать</div>\
        </div>\
    ');
    ...
    return function() {
        ...
        var Process = function(self) {
            ...
            if ( typeof BackupCustomName !== 'string' || BackupCustomName.length === 0 ) {
                ...
                self.html('<span class="Error">Неверное имя!</span>');
            ...
            self.html('<span class="success nomargin">Сохранено!</span>');
            self.find('span').fadeOut(2500, function () {
                $(this).remove();
                self.html(label);
            });
...
setuptools.app.backups.create = function(silent) {
    ...
    setuptools.lightbox.build('backups-create', ' \
        Резервная копия была создана с именем ' + BackupObject.BackupName + '. \
        <br><br><span class="setuptools config download acknowledge">Используйте правый клик мыши и нажмите \'Save link as...\' для загрузки.</span> \
    ');
    ...
    setuptools.lightbox.build('backups-create', ' \
        <div class="cfleft">\
            <br><a download="' + BackupObject.BackupName + '.json" href="data:text/json;base64,' + btoa(BackupObject.BackupData) + '" class="setuptools config download noclose">Скачать</a>\
        </div>\
    ');

    //  display the download box
    if ( setuptools.state.firsttime === true ) setuptools.lightbox.build('backups-create', '<br><br>Добро пожаловать в Muledump! :)<br><br>Если вы готовы, вы можете <a href="#" class="setuptools link reload noclose">приступить</a> к использованию.');
    if ( setuptools.state.firsttime === false ) setuptools.lightbox.goback('backups-create', setuptools.app.backups.index);
    setuptools.lightbox.settitle('backups-create', 'Создание резервной копии');
